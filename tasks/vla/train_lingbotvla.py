# -*- coding: utf-8 -*-
"""
train_lingbotvla.py —— LingBot-VLA 2.0 训练主循环(项目最核心文件)
====================================================================
【定位】被 train.sh → torchrun 拉起,每个 GPU 进程跑一份本文件,
       完成"建模型 → 喂数据 → 前向 → 算loss → 反向 → 更新 → 存checkpoint"全流程。

【main() 的 10 个阶段】(大致行号)
  ① 参数解析 + 分布式初始化       parse_args / init_process_group          (~332)
  ② checkpointer + 并行状态       build_checkpointer / init_parallel_state (~345)
  ③ 构建 VLA 模型 + depth/video教师 build_foundation_model / build_depth_model (~359)
  ④ processor + 数据集 + DataLoader  build_processor / build_vla_dataset / build_dataloader (~423)
  ⑤ 模型并行(FSDP2)+ compile     build_parallelize_model / torch.compile  (~479)
  ⑥ 优化器 + MoE负载均衡 + LR调度   build_muon_optimizer / build_optimizer / build_lr_scheduler (~508)
  ⑦ 日志(TensorBoard/wandb)+资产  AsyncTBWriter / save_model_assets        (~574)
  ⑧ 断点恢复(resume)            Checkpointer.load                        (~644)
  ⑨ ★训练主循环                  for epoch → for step → 前向/反向/更新     (~719)
  ⑩ checkpoint 保存 + 清理        Checkpointer.save                        (~1095)

【训练循环每一步(阶段⑨的核心)】
  取 micro_batch → depth/video 教师算蒸馏目标 → model 前向算 loss → loss.backward()
  → clip_grad_norm → optimizer.step → lr_scheduler.step → all_reduce 统计 → 记日志/存 ckpt

【核心调用方法(build_* 系列)】
  build_foundation_model      建 VLA 模型(Qwen3-VL 骨干 + MoE action expert)
  build_depth_model           建 depth 教师网络(MoGe + MoRGBD,几何蒸馏)
  build_video_model           建 video-DINO 教师网络(未来帧预测蒸馏)
  build_vla_dataset           建 VLA 数据集(MultiVLADataset / VLADataset)
  build_dataloader            建 DataLoader(支持动态 batch)
  build_parallelize_model     给模型套 FSDP2 并行壳
  build_optimizer / muon      建优化器(AdamW / Muon)
  build_lr_scheduler          建学习率调度器
  build_checkpointer          建 checkpoint 存取器(DCP)
  build_moe_load_balance_hook MoE 无损(loss-free)负载均衡 hook
====================================================================
"""
...
def main():
    ...
    if candidates:
        ...
        if not loaded:
            if args.train.load_checkpoint_path:
                # 显式指定的热启动档加载失败(路径写错/结构不符/文件损坏)—— 直接报错终止。
                # 不能沿用"静默从头训"的兜底:续训场景下从头训要白烧几十小时 GPU,
                # 而且日志里只有一行 info,极易漏看(v4_w 相位加权续训特意加的保险)。
                raise RuntimeError(
                    f"load_checkpoint_path 指定的 checkpoint 加载失败,拒绝静默从头训练: "
                    f"{load_checkpoint_path}(最后错误: {last_err!r})"
                )
    ...
    # ==================== ⑨ 训练主循环(全文件核心)====================
    # 结构:for epoch → for step(global_step++) → for micro_batch(梯度累积的若干微批)
    # 每个 global_step 做五件事:
    #   1) 取一批 micro_batches(梯度累积)
    #   2) 逐 micro_batch:depth/video 教师算蒸馏目标 → model 前向算 loss → loss.backward()
    #   3) clip_grad_norm → optimizer.step → lr_scheduler.step → zero_grad
    #   4) all_reduce 汇总各卡 loss → 打印日志 → 写 TensorBoard
    #   5) 到 save_steps 存 checkpoint;到 max_steps 结束
    for epoch in range(start_epoch, args.train.num_train_epochs):
        ...
        for epoch_step in range(start_step, args.train.train_steps):
            ...
            for micro_batch in micro_batches:
                ...
                # 纯 bf16(enable_mixed_precision=false)时模型权重是 bf16,而数据管线输出
                # fp32 的 state/actions/images —— 统一 cast 成模型参数 dtype,避免
                # "mat1 and mat2 must have the same dtype" 报错。
                # mixed_precision=true 时模型已被 .float() 成 fp32,此 cast 为 no-op。
                _input_dtype = next(model.parameters()).dtype
                micro_batch = {
                    k: (
                        v.cuda(non_blocking=True).to(_input_dtype)
                        if isinstance(v, torch.Tensor) and v.is_floating_point()
                        else v.cuda(non_blocking=True) if isinstance(v, torch.Tensor) else v
                    )
                ...
                # ---- 模型前向 + 算 loss ----
                # model() 内部:VLM 编码图像/语言 → MoE action expert(flow matching)预测动作 chunk
                #   + depth/video 蒸馏 loss(若 use_depth_align)。返回多个 loss 分量。
                # model_outputs 长度随蒸馏开关变化(6/8/10/11),分别对应不同 loss 组合。
            ...
            # ---- 梯度裁剪 + 优化器更新(所有 micro_batch 梯度已累积完毕)----
            optimizer.step()
            lr_scheduler.step()
            optimizer.zero_grad()
            ...
            # ---- ⑩ 存 checkpoint(每 save_steps 步)----
            # state 含 model/optimizer/lr_scheduler/dataloader/RNG 状态,支持断点续训。
            # 随后异步转成 HF 格式权重(save_hf_checkpoint_best_effort),供推理直接加载。
