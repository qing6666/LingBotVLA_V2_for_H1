...
def fused_moe_forward(
    ...
    # Unified bf16 cast — group_gemm kernel requires all operands to be bf16/fp16.
    # Cast before the EP/non-EP branch so both paths are covered (including
    # fused + ep_size=1 + enable_fp32=true). Weights must also be cast because
    # they may be fp32 when enable_fp32=true.
    compute_dtype = torch.bfloat16
    hidden_states = hidden_states.to(compute_dtype)
    routing_weights = routing_weights.to(compute_dtype)
    fc1_1_weight = fc1_1_weight.to(compute_dtype)
    fc1_2_weight = fc1_2_weight.to(compute_dtype)
    fc2_weight = fc2_weight.to(compute_dtype)

    # FSDP2 all-gathered weights are DTensor — triton kernels cannot handle
    # DTensor metadata, so convert to plain tensors via to_local() which is
    # autograd-aware (unlike ._local_tensor) to preserve gradient flow.
    from torch.distributed._tensor import DTensor as _DTensor
    if isinstance(hidden_states, _DTensor):
        hidden_states = hidden_states.to_local()
    if isinstance(fc1_1_weight, _DTensor):
        fc1_1_weight = fc1_1_weight.to_local()
    if isinstance(fc1_2_weight, _DTensor):
        fc1_2_weight = fc1_2_weight.to_local()
    if isinstance(fc2_weight, _DTensor):
        fc2_weight = fc2_weight.to_local()

    if get_parallel_state().ep_enabled:
        # EP path — runs in both train and eval (each rank only has E/ep_size experts,
        # token dispatch is always needed). Removed module.training gate per plan.
        ...
        # Non-EP fused path (group_gemm without token dispatch)
