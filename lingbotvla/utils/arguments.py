...
"""arguments.py —— 全局参数定义与解析(LingBot-VLA 2.0 的"参数中枢")
====================================================================
【定位】所有训练/数据/模型/评估参数都在这里以 dataclass 定义,
       并由 parse_args() 把"命令行 + yaml 配置文件"合并成一套参数对象。
       train_lingbotvla.py / compute_norm_stats.py 都靠它拿参数。

【6 个参数类】
  ModelArguments    模型相关(权重路径、config_key、MoE 实现方式等)
  DataArguments     数据相关(数据集路径、joints、cameras、norm_type 等)
  TrainingArguments 训练相关(超大!lr/batch/optimizer/FSDP/MoE/checkpoint 全在这)
  EvalArguments     评估相关
  InferArguments    推理相关
  (train_lingbotvla.py 继承它们 → MyTrainingArguments / MyDataArguments)

【2 个核心函数】
  parse_args(rootclass) 把 sys.argv(命令行 + yaml)解析成 rootclass 实例
                         机制:yaml 提供基础值 → 命令行 --base.name=value 覆盖(优先级更高)
  save_args(args, dir)   把参数存成 lingbotvla_cli.yaml(推理时 deploy 靠它找回训练配置)

【参数说明权威文档】docs/config/lingbotvla_config_doc.md(每个参数的英文说明)
====================================================================
"""
...
class DataArguments:
    ...
    phase_window_file: str = field(
        default="",
        metadata={
            "help": (
                "Phase-weighted sampling: path to a .phase_windows.json (absolute-row "
                "ranges + multiplicity) generated by make_phase_windows.py. Empty = "
                "uniform sampling (bit-identical to the historical behavior)."
            )
        },
    )
    def __post_init__(self):
...
class TrainingArguments:
    ...
    resume_dataloader_state: bool = field(
        ...
        metadata={
            "help": (
                "Restore the saved dataloader/sampler position when loading a checkpoint. "
                "Keep true for exact resume; set false when the dataset sampling space "
                "changed since the checkpoint was written (e.g. phase-weighted "
                "expansion) so the sampler starts fresh over the new index space."
            )
        },
    )
    ...
    action_loss_weights: Optional[List[float]] = field(
        default=None,
        metadata={"help": "Per-dim weights on the (padded) unified action-space loss. "
                          "Length must equal max_action_dim (55). None = uniform weighting "
                          "(bit-identical to the old behavior). v4: upweight right arm + "
                          "grippers (v3 measured bias dims), zero the constant head dims."},
    )
...
def parse_args(rootclass: T) -> T:
    ...
    """
    # ===== parse_args 流程(5 步)=====
    # 1) 遍历 rootclass 的每个子 dataclass,把每个 field 注册成命令行参数 --base.name
    # 2) 读第一个参数(yaml/json 配置文件)→ input_data
    # 3) 合并:yaml 值转成 --base.name value 追加;命令行已有的 --base.name= 不覆盖(命令行优先)
    # 4) argparse 解析 → 按 base 分组
    # 5) 实例化各子 dataclass → 组装 rootclass 返回
    ...
    # ===== 第 2-3 步:读配置文件 + 合并命令行 =====
    cmd_args = sys.argv[1:]
    cmd_args_string = "=".join(cmd_args)  # use `=` to mark the end of arg name  用 = 标记参数名结束(便于判断命令行是否覆盖)
    input_data = {}
    ...
    # ★ 合并优先级:命令行 > 配置文件
    # 把 yaml 里每个参数转成 "--base.name value" 追加到 cmd_args;
    # 但若命令行已有 "--base.name=xxx"(在 cmd_args_string 里),则跳过 → 命令行覆盖 yaml。
    for base, arg_dict in input_data.items():
        for arg_name, arg_value in arg_dict.items():
            if f"--{base}.{arg_name}=" not in cmd_args_string:  # lower priority  命令行没覆盖,才用 yaml 的值
...
def save_args(args: T, output_path: str) -> None:
    ...
    把参数对象存成 lingbotvla_cli.yaml(训练启动时由 rank0 调用)。
    推理时 deploy / open_loop_eval 通过读这个 yaml 找回训练配置
    (如 config_key、tokenizer_path),从而知道用哪个策略模块、怎么归一化。
    """
