    ...
    def add(self, micro_batch: Dict[str, "torch.Tensor"]) -> None:
        ...
        if "image_grid_thw" in micro_batch:
            ...
            if image_grid_thw.ndim > 2:
                if "img_masks" in micro_batch:
                    image_grid_thw = image_grid_thw[micro_batch["img_masks"].to(dtype=torch.bool)]
                else:
    ...
    for key, value in example.items():
        if isinstance(value, torch.Tensor):
            if value.numel() <= 200:
                ...
                logger.info(
                    f"[rank {rank}]: {key}'s shape: {value.shape}, "
                    f"device: {value.device}, dtype: {value.dtype}, numel: {value.numel()}"
                )
        else:
