# -*- coding: utf-8 -*-
"""
qwen2_action_expert.py —— MoE 动作专家(基于 Qwen2 改造)
====================================================================
【作用】LingBot-VLA 2.0 的 action expert。把"一个大脑处理所有动作"
       换成"32 个专家 + 路由器",每个 token 只激活 4 个 → 参数容量大又省算力。

【核心组件】
  Qwen2MoeRoutedExpertMLP    单个"路由专家"(小 MLP)
  Qwen2MoeSharedExpertMLP    "共享专家"(所有 token 都过,提取通用知识)
  Qwen2FusedExperts          融合专家:32 个专家权重存成 3D 张量,用 fused_moe/group_gemm 高效算
  Qwen2TokenMoeBlock         ★ MoE 路由核心:gate 打分 → top_k(32选4) → 调专家 → +共享专家
  Qwen2DecoderLayer          一层 decoder(self-attention + MoE MLP)
  Qwen2Model/ForCausalLM     堆叠多层 decoder = action expert 主体

【MoE 路由流程(Qwen2TokenMoeBlock.forward)】
  1) gate 给每个 token 对 32 专家打分(router_logits,fp32 防 bf16 抖动)
  2) softmax/sigmoid + e_score_correction_bias(loss-free 均衡偏置)
  3) top_k 选 4 个专家,取权重并归一化
  4) 融合计算(fused_moe / robby_moe 推理加速)或 eager 逐专家算
  5) + 共享专家输出(所有 token 共享的通用知识)
  6) 返回 final_hidden + router_logits(给 MoE 均衡 loss 用)

【被谁用】modeling_lingbot_vla_v2.py 的 QwenvlWithExpertV2Model 安装这些 MoE 块
====================================================================
"""
...
# ---- 融合专家:把 E 个专家的 gate/up/down 权重存成 3D 张量 [E, ..., ...] ----
# 用 fused_moe / group_gemm 一次性算所有专家(比逐专家循环快很多)。
# FSDP2 友好:forward 经 self.experts(...) 调用,触发 FSDP2 unshard 参数。
...
# ==================== ★ MoE 路由核心:32 选 top_k(默认4)====================
# forward 流程:
#   gate 打分(router_logits,用 fp32 防 bf16 抖动)→ 激活(sigmoid/softmax)
#   → + e_score_correction_bias(loss-free 均衡偏置,被 optimizer pre-hook 更新)
#   → top_k 选专家 + 取权重 + 归一化 + 缩放
#   → 专家计算(fused 融合 / eager 逐个 / robby_moe 推理加速)
#   → + 共享专家(所有 token 共享的通用知识)
#   → 返回 final_hidden + router_logits(后者给 MoE 均衡 loss 用)
# 缓冲区 tokens_per_expert:各专家负载,给均衡 hook 监控/调 bias 用。
...
# ==================== Qwen2ForCausalLM:action expert 主体 ====================
# 堆叠多层 Qwen2DecoderLayer(每层 = self-attention + MoE MLP)。
# 这就是 LingBot-VLA 2.0 的"动作专家":接在 VLM 后面,用 MoE 推理出动作。
# 被 modeling_lingbot_vla_v2.py 的 QwenvlWithExpertV2Model 安装为 qwen_expert。
