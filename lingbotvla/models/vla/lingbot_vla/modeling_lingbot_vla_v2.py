# -*- coding: utf-8 -*-
"""
modeling_lingbot_vla_v2.py —— LingBot-VLA 2.0 主模型(VLA 架构核心)
====================================================================
【三段架构】
  1) QwenvlWithExpertV2Model  VLM 骨干 = Qwen3-VL(图像+语言) + MoE action expert
                              forward():VLM 前向,产出 hidden states + router logits
  2) FlowMatchingV2           ★ 动作生成核心(继承 FlowMatchingV1),含上面的骨干
       · forward()         训练:flow matching 算 vla_loss + depth/video 蒸馏 loss
       · sample_actions()  推理:从纯噪声出发,ODE 积分 num_steps 步去噪 → 动作
       · predict_velocity() 去噪一步:预测速度场 v(x_t,t),复用 prefix KV cache
  3) LingbotVlaV2Policy       训练/推理统一封装(PreTrainedModel 子类),deploy 入口
       · forward()         训练:调 FlowMatchingV2.forward → 加权合成 total_loss
       · sample_actions()  推理:转发

【Flow Matching 动作生成原理】
  训练:在「真实动作 a」和「噪声 ε」之间线性插值 x_t = t·ε + (1-t)·a,
        目标速度 u_t = ε - a;模型学预测 v_t ≈ u_t,loss = MSE(u_t, v_t)。  ← vla_loss
  推理:从 x_1=纯噪声 出发,沿预测速度 v 积分 x_{t-dt} = x_t + dt·v_t,
        num_steps 步后得到 x_0 ≈ 真实动作。(欧拉法求解 ODE)

【被谁调用】
  train_lingbotvla.py: build_foundation_model → LingbotVlaV2Policy
                       训练循环 model(**batch) → Policy.forward → FlowMatchingV2.forward
  deploy/policy.py:    加载 Policy → sample_actions() 生成动作
====================================================================
"""
...
class FlowMatchingV2(FlowMatchingV1):
    ...
    # ==================== 训练前向:算 vla_loss + 蒸馏 loss ====================
    # Flow Matching 核心:
    #   x_t = t·noise + (1-t)·actions   (在噪声和真实动作间线性插值)
    #   u_t = noise - actions           (目标速度场)
    #   模型预测 v_t,loss = MSE(u_t, v_t)  ← 这就是 vla_loss
    # 同时(若 align_params)算 depth/video 蒸馏 loss;并产出 MoE 的 seq_wise/z_loss。
    def forward(
        self,
        images,
        img_masks,
    ...
    # ==================== 推理:从噪声去噪生成动作(ODE 积分)====================
    # 1) 前向 prefix(图像+语言)一次,缓存 KV cache(后续去噪步骤复用)
    # 2) x_t = 纯噪声;time 从 1.0 → 0.0,步长 dt = -1/num_steps
    # 3) 每步:predict_velocity → v_t;x_t += dt·v_t(欧拉法积分)
    # 4) num_steps 步后 x_t ≈ 真实动作。(README 说 4090D 上 10 步去噪约 130ms)
    def sample_actions(
        self,
        images,
        img_masks,
    ...
    # ==================== 去噪一步:预测速度场 v(x_t, t) ====================
    # 关键优化:prefix(图像+语言)的 KV cache 已在 sample_actions 算好,
    #          这里只前向 suffix(state + x_t + time),大幅省算力(去噪循环里不用重算图像)。
...
# ==================== LingbotVlaV2Policy:训练/推理统一封装(deploy 入口)====================
# train_lingbotvla.py 和 deploy/policy.py 都用它做入口。
# · forward():训练时调 FlowMatchingV2.forward 拿各 loss 分量 → 加权合成 total_loss
#              (多 loss 合成 total_loss = vla + depth + future_depth + future_video
#               + seq_wise + z_loss,就在这里!);用 joint_mask 屏蔽 padding 维度。
# · sample_actions():推理时转发给 FlowMatchingV2.sample_actions。
class LingbotVlaV2Policy(PreTrainedModel):
    ...
    def __init__(self, config: LingbotVLAV2Config, eval: bool = False):
        ...
        # v4 per-dim 动作损失权重(统一动作空间 55 维;None=不加权,与历史行为逐比特一致)。
        # persistent=False:不进 state_dict/checkpoint,旧权重加载和推理侧完全不受影响。
        _w = getattr(config, "action_loss_weights", None)
        if _w is not None:
            assert len(_w) == config.max_action_dim, (
                f"action_loss_weights 长度 {len(_w)} != max_action_dim {config.max_action_dim}")
            self.register_buffer(
                "action_loss_weight_vec", torch.tensor(_w, dtype=torch.float32), persistent=False
            )
    ...
    def forward(
        ...
        **kwargs
        ...
        # v4 per-dim 加权:在 joint_mask / 归约之前乘到逐元素 losses 上。
        # MoE 均衡损失已在 self.model.forward 内部用未加权 losses 算好,不受影响。
        _w = getattr(self, "action_loss_weight_vec", None)
        if _w is not None:
            losses = losses * _w.to(device=losses.device, dtype=losses.dtype)
