...
# 可选蒸馏模块的参数前缀:这些模块仅在训练配置里写了 align_params(开蒸馏)时才构建。
# 关闭蒸馏做 post-training 时模型里没有它们,checkpoint 中的对应键应跳过而不是报错;
# 开蒸馏时模块存在、键正常加载,此白名单不生效。
OPTIONAL_DISTILL_PREFIXES = (
    ...
    for state_dict_iterator in tqdm(
        ...
        for ckpt_key, tensor in state_dict_iterator:
            ...
            if name in buffer_dict:
                ...
                if post_training:
                    if name.startswith(OPTIONAL_DISTILL_PREFIXES):
                        # 关蒸馏时模型未构建蒸馏模块,checkpoint 里的蒸馏头权重跳过
                        logger.info_rank0(f">>> Skip distill-only key (distillation disabled): {name}")
                        continue
