...
# =====================================================================
# muon.py —— Muon 优化器(梯度正交化,DTensor/FSDP2/MoE 感知)
# ---------------------------------------------------------------------
# 【Muon 是什么】一种新优化器:对 2D 权重矩阵的梯度,先用 Newton-Schulz 迭代
#   "正交化"(取梯度的正交方向),再用动量更新。正交化后各方向等比例,更新更均衡。
# 【核心组件】
#   batched_newton_schulz   Newton-Schulz 迭代,快速逼近矩阵正交化(Muon 的数学核心)
#   split_muon_adamw_params 把参数分组:2D 权重→Muon,1D(embedding/lm_head/norm)→AdamW
#   DistributedMuon         ★ 优化器主体:动量 + 正交化梯度 + weight decay
# 【分布式】FSDP2 的 2D 参数 mega-batch(同形状堆叠一次通信);MoE 3D 专家栈批量化。
# 【被谁用】train_lingbotvla.py 里 optimizer="muon" 时用;否则用 AdamW。
# =====================================================================
...
# ---- Newton-Schulz 迭代:快速逼近矩阵正交化(Muon 的数学核心)----
# 把梯度 G 正交化成 U(类似 SVD 取正交部分),5 步迭代即可,比 SVD 快得多。
# 正交化后的梯度各方向等比例 → 权重更新更均衡 → 收敛更好。
@torch.no_grad()
...
# ---- 参数分组:2D 权重走 Muon,1D/特定名(embedding/lm_head/norm)走 AdamW ----
# 因为 Muon 的正交化只对矩阵(2D+)有意义,1D 参数(偏置/norm)用 AdamW。
...
# ==================== DistributedMuon:Muon 优化器主体 ====================
# step 流程(每个参数):
#   1) 动量:buf = lerp(buf, grad)            (SGD/Nesterov 动量缓冲)
#   2) 正交化:ortho = newton_schulz(update)  (★ Muon 的核心,仅 2D/3D 做)
#   3) 调 LR:_adjust_lr(match_rms_adamw 让更新幅度和 AdamW 匹配)
#   4) weight decay + 更新:p -= lr * ortho
# 分布式:FSDP2 2D 参数 mega-batch(同形状堆叠一次 NCCL);MoE 3D 专家 gather 后正交化。
