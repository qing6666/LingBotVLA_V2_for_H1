...
_FUSED_EXPERT_PATTERN = _re.compile(r'^(.+\.mlp)\.experts\.(gate_proj|up_proj|down_proj)$')


def fused_to_per_expert_state_dict(state_dict: dict) -> dict:
    """Convert fused 3D expert params to per-expert HF format.

    experts.gate_proj [E, I, H] -> experts.{j}.gate_proj.weight [I, H]
    experts.up_proj   [E, I, H] -> experts.{j}.up_proj.weight   [I, H]
    experts.down_proj [E, H, I] -> experts.{j}.down_proj.weight [H, I]
    """
    new_state_dict = {}
    for key, tensor in state_dict.items():
        match = _FUSED_EXPERT_PATTERN.match(key)
        if match and tensor.dim() == 3:
            prefix, proj_name = match.groups()
            for j in range(tensor.shape[0]):
                new_key = f"{prefix}.experts.{j}.{proj_name}.weight"
                new_state_dict[new_key] = tensor[j]
        else:
